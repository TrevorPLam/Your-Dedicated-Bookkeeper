# Your Dedicated Bookkeeper - Service Offerings

> **Transition note:** The service definitions below are legacy marketing offerings from the pre-transition site. Bookkeeping service tiers are still being defined and should replace this content once finalized.

## Purpose
Drive growth through structured marketing systems, content engines, acquisition funnels, and analytics.

---

## Core Services

### Content Planning & Publishing
- Monthly content calendar development
- Blog post creation and optimization
- Social media content creation
- Editorial workflow management
- Content distribution strategy

### SEO Foundations
- Technical SEO audits
- On-page optimization
- Keyword research and mapping
- Meta tag optimization
- Site structure recommendations
- Local SEO setup

### Social Media Management (Organic)
- Platform strategy development
- Post scheduling and publishing
- Community management
- Engagement monitoring
- Social media calendar

### Email Campaigns
- Campaign planning and strategy
- Email copywriting
- List segmentation
- Basic automation setup
- Performance tracking

---

## Support Services

### Ad Account Setup
- Platform account configuration
- Pixel and tracking implementation
- Conversion tracking setup
- Audience creation
- Campaign structure planning

### CRM Setup & Management
- CRM platform selection
- Tagging and tracking architecture
- Contact import and organization
- Basic workflow automation
- Reporting dashboard setup

### Funnel Build-Out
- Landing page optimization
- Lead magnet creation
- Basic funnel mapping
- Form and conversion tracking
- A/B test setup

### Marketing Reporting
- Analytics dashboard creation
- Monthly performance reports
- KPI tracking and analysis
- Campaign attribution
- ROI measurement

### Creative Coordination
- Brief development for designers
- Vendor management
- Asset review and approval
- Brand guideline enforcement
- Creative file organization

---

## Advisory Services

### Marketing Strategy Planning
- Quarterly marketing plan development
- Channel strategy recommendations
- Budget allocation guidance
- Competitive analysis
- Growth opportunity identification

### Brand Messaging Development
- Brand voice and tone definition
- Value proposition refinement
- Messaging framework creation
- Positioning statement development
- Audience persona development

### Funnel Architecture
- Customer journey mapping
- Conversion optimization strategy
- Funnel performance analysis
- Multi-channel attribution modeling
- Growth experimentation framework

### Quarterly Marketing Review (QMR)
- Performance analysis and insights
- Strategy refinement
- Goal setting and OKRs
- Budget review and optimization
- Team alignment sessions

---

## Add-Ons

### PPC Management
- Google Ads campaign management
- Social media advertising
- Display and retargeting campaigns
- Budget optimization
- Advanced bid strategies

### Website Redesign Projects
- User experience design
- Conversion rate optimization
- Technical implementation
- Content migration
- Launch and testing

### Advanced SEO Programs
- Link building campaigns
- Content hub development
- Technical SEO implementation
- Schema markup
- International SEO

### Video Content Packages
- Video content strategy
- Script development
- Video editing coordination
- YouTube optimization
- Video distribution

### Marketing Automation
- Advanced workflow development
- Lead scoring implementation
- Multi-touch attribution
- Behavioral email sequences
- CRM integration

---

## Out of Scope

### Public Relations
- Media relations
- Press release distribution
- Crisis communications
- Spokesperson training
- Media monitoring

### Full-Service Creative Agency Work
- Brand identity design (logos, brand books)
- Video production
- Photography
- Print design
- Packaging design

### Enterprise CRO Programs
- Advanced multivariate testing
- Enterprise personalization
- Heat mapping and session recording
- Advanced analytics implementation

### Direct Sales Activities
- Sales calls or meetings
- Deal negotiation
- Contract development
- Sales team management

---

## Service Tiers

### Starter
**Best for**: Early-stage businesses needing foundational marketing
- Content planning (4 posts/month)
- Basic SEO optimization
- Social media setup (2 platforms)
- Email campaign support (2/month)
- Monthly performance report

### Growth
**Best for**: Established businesses ready to scale
- All Starter services
- Advanced SEO program
- Email automation
- CRM management
- Ad account setup and monitoring
- Funnel optimization
- Bi-weekly strategy calls

### Scale
**Best for**: Businesses with complex marketing needs
- All Growth services
- PPC management
- Marketing automation
- Advanced funnel architecture
- Quarterly marketing reviews
- Weekly strategy calls
- Dedicated account manager

---

## Engagement Models

### Retainer (Monthly)
- Ongoing services with consistent deliverables
- Predictable monthly investment
- Priority support and communication
- Most common for Core and Support services

### Project-Based
- Defined scope and deliverables
- Fixed timeline and budget
- Best for Add-Ons and one-time initiatives
- Examples: Website redesigns, SEO audits, funnel builds

### Advisory (Quarterly)
- Strategic guidance and planning
- Quarterly reviews and planning sessions
- Async communication between sessions
- Best for businesses with in-house marketing teams

---

## Getting Started

1. **Discovery Call**: 30-minute conversation to understand your business and goals
2. **Marketing Audit**: Review current marketing efforts and identify opportunities
3. **Strategy Proposal**: Custom plan with recommended services and timeline
4. **Onboarding**: Kickoff meeting, access setup, and initial deliverables planning
5. **Execution**: Regular delivery of services with ongoing communication and optimization
